...
router.post('/register', (req,res)=>{
  ...
  if(error) res.status(403).send({"message":error.details[0].message});
  const query = {
  ...
  client.query(query, async(err,r)=>{
    if(r.rowCount) res.send({ "message":"User already exists" });
...
router.post('/login',function(req,res){
  ...
  client.query(query,async (err,r)=>{
    if(err) res.status(403).send({"message":"Something sewnt wrong"});
    if(!r.rowCount) res.send({"message":"No user exists with this email id"});
    //Check if password is correct
    const validPass = await bcrypt.compare(req.body.password,r.rows[0]['password']);
    if(!validPass) res.json("Wrong password");
    const user = r.rows[0];
    const token = jwt.sign({_id:user.user_id,
                             username: user.username,
                           },process.env.TOKEN_SECRET,{expiresIn: '5days'});
    res.header('auth-token',token).json("Token present in header as auth-token");
    })
})
